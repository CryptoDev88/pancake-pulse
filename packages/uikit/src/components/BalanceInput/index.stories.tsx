...
export const SwitchUnits: React.FC<React.PropsWithChildren> = () => {
  const CAKE_PRICE = 69;
  const [editingUnit, setEditingUnit] = useState<"DEX" | "USD">("DEX");
  const conversionUnit = editingUnit === "DEX" ? "USD" : "DEX";
  ...
  const switchEditingUnits = () => {
    const editingUnitAfterChange = editingUnit === "DEX" ? "USD" : "DEX";
  ...
  const handleCakeChange = (input: string) => {
    ...
    if (editingUnit === "DEX") {
      ...
      setValues({
        DEX: Number.isNaN(inputAsFloat) ? "" : `${inputAsFloat / CAKE_PRICE}`,
        USD: input,
      });
    }
  };
