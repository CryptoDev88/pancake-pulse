...
export default function RemoveStableLiquidity({ currencyA, currencyB, currencyIdA, currencyIdB }, history) {
  ...
  const handleSelectCurrencyA = useCallback(
    (currency: Currency) => {
      if (currencyIdB && currencyId(currency) === currencyIdB) {
        history.replace(`/remove/${currencyId(currency)}/${currencyIdA}`)
      } else {
        history.replace(`/remove/${currencyId(currency)}/${currencyIdB}`)
      }
    },
  ...
  const handleSelectCurrencyB = useCallback(
    (currency: Currency) => {
      if (currencyIdA && currencyId(currency) === currencyIdA) {
        history.replace(`/remove/${currencyIdB}/${currencyId(currency)}`)
      } else {
        history.replace(`/remove/${currencyIdA}/${currencyId(currency)}`)
      }
    },
    [currencyIdA, currencyIdB, history],
  )
