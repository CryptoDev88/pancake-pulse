...
export function useDefaultsFromURLSearch():
  ...
  const parsedQueryParams: ParsedUrlQuery = Array.from(queryParams.entries()).reduce((acc, [key, value]) => {
    acc[key] = value;
    return acc;
  }, {} as ParsedUrlQuery);
  ...
  useEffect(() => {
    ...
    const parsed = queryParametersToSwapState(parsedQueryParams, native.symbol, DEX[chainId]?.address ?? USDC[chainId]?.address)
  ...
  }, [dispatch, chainId, parsedQueryParams, native])
