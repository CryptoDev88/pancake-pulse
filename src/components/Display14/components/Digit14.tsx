...
const letters = ['A', 'B', 'C', 'D', 'E', 'F', 'G', 'N', 'H', 'I', 'J', 'K', 'L', 'M', 'O'] as const
...
export const Digit14 = ({ char = '-', color = 'red', height = 250, skew = false }: DigitType) => {
  const style = {
    height: `${height}px`,
    width: `${height * 0.7}px`,
    zIndex: '1',
  ...
  return (
    <div className="digit" style={style}>
      {activeArray.map((active, index) => {
        ...
        return <Segment key={letter} active={active === 1} size={height / 12.5} color={color} id={letter} skew={skew} />
      })}
    </div>
  )
}
